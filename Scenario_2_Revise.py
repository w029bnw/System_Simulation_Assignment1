...
class DriveThru_SC2(object):
    ...
    def order_pay(self, name, order_t):
        ...
        f.write("%7.4f: %s has paid for their order.\n" % (self.env.now, name))
    ...
    def pickup(self, name, pick_up_t):
        ...
        f.write("%7.4f: %s has recieved their order and is leaving the line.\n" %(self.env.now, name))
...
def customer(env, name, drive_thru):
    ...
    if(len(drive_thru.station1.queue) < BALK_LIMIT or len(drive_thru.station2.queue) < BALK_LIMIT):
        ...
        """
        Activate Scenario 2 FLoaters will enter the environment
        Floaters have unique trait to both take orders and recieve payment
        """
        f.write('%s balked.\n' % (name))
    
def customer_s2(env, name, drive_thru):
    """
    ...
    if(len(drive_thru.station1.queue) < BALK_LIMIT or len(drive_thru.station2.queue) < BALK_LIMIT):
        ...
        if(drive_thru.station1.count == 0 or (len(drive_thru.station1.queue) <= len(drive_thru.station2.queue) and (drive_thru.station1.count == 1 and drive_thru.station2.count == 1))):
            ...
            f.write('%7.4f: %s has finished ordering 1.\n' % (env.now, name))
            ...
            f.write('%7.4f: %s has finished ordering.\n' % (env.now, name))
...
def setup(env):
    ...
    drive_thru = DriveThru_SC2(env)
    
    # Create more customers while the simulation is running
    i = 0
    while env.now < SIM_TIME:
        env.process(customer_s2(env, 'Customer%02d' % i, drive_thru))
